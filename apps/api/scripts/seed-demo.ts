...
import {
  guayaquilToday,
  parseCivilDate,
} from "../src/modules/operational/operational-time.functions";
...
const stopDefinitions = [
  {
    name: `${PREFIX} · Avícola Fernández`,
    latitude: -2.1423125,
    longitude: -79.882671875,
  },
  {
    name: `${PREFIX} · Confecciones Don Lucho`,
    latitude: -2.1444375,
    longitude: -79.892984375,
  },
  {
    name: `${PREFIX} · Colegio Americano`,
    latitude: -2.1384375,
    longitude: -79.9303125,
  },
  {
    name: `${PREFIX} · Paso Peatonal Puerto Azul`,
    latitude: -2.1917875,
    longitude: -79.972359375,
  },
  {
    name: `${PREFIX} · Mi Comisariato Vía a la Costa`,
    latitude: -2.1818125,
    longitude: -79.995859375,
  },
  {
    name: `${PREFIX} · UPS Campus María Auxiliadora`,
    latitude: -2.198611111,
    longitude: -80.041944444,
  },
] as const;
...
const ensureStop = async (
  ...
  if (existing) {
    await prisma.stop.update({
      ...
      data: { latitude, longitude, isActive: true },
    });
    return existing.id;
  }
...
const ensureRoutePath = async (
  ...
  const existing = await prisma.routePath.findUnique({
    where: { serviceLineId_code: { serviceLineId, code } },
  });
  const routePath = existing
    ? await prisma.routePath.update({
        where: { id: existing.id },
        data: { displayName, direction, isActive: true },
      })
    : await prisma.routePath.create({
        data: { serviceLineId, code, displayName, direction },
      });
...
const ensurePattern = async (
  ...
  const existing = await prisma.schedulePattern.findFirst({
    where: { serviceCalendarId: calendarId, direction, exceptionId: null },
  });
  const pattern = existing
    ...
    : await prisma.schedulePattern.create({
        data: { serviceCalendarId: calendarId, direction, status: "PUBLISHED" },
      });
  await prisma.schedulePatternDay.upsert({
    where: {
      schedulePatternId_weekday: { schedulePatternId: pattern.id, weekday },
    },
    update: {},
...
const ensureTimeAndJourney = async (
  ...
  const scheduleTime = await prisma.scheduleTime.upsert({
    where: {
      schedulePatternId_departureTime: {
  ...
  for (const [index, routePathStop] of routePathStops.entries()) {
    const offsetMinutes =
      index === 0
        ? 0
        : Math.round(
            (finalOffsetMinutes * index) / (routePathStops.length - 1),
          );
    await prisma.scheduledStopTime.upsert({
      where: {
        journeyTemplateId_routePathStopId: {
...
const removeDemoRuns = async (assignmentIds: string[]): Promise<void> => {
  ...
  const runs = await prisma.serviceRun.findMany({
    where: { serviceAssignmentId: { in: assignmentIds } },
    select: { id: true },
  });
...
const ensureDataset = async (): Promise<void> => {
  ...
  if (!serviceDate || !parsed.ok)
    throw new Error("Unable to calculate UPS GO demo civil date");
  ...
  const campus = await prisma.campus.upsert({
    ...
    create: {
      code: campusCode,
      name: "Campus María Auxiliadora · Demo UPS GO",
    },
  });
  const existingLine = await prisma.serviceLine.findUnique({
    where: { campusId_code: { campusId: campus.id, code: lineCode } },
  });
  const line = existingLine
    ? await prisma.serviceLine.update({
        where: { id: existingLine.id },
        data: { name: "Ruta Norte", isActive: true },
      })
    : await prisma.serviceLine.create({
        data: { campusId: campus.id, code: lineCode, name: "Ruta Norte" },
      });

  const stopIds = await Promise.all(
    stopDefinitions.map((stop) =>
      ensureStop(stop.name, stop.latitude, stop.longitude),
    ),
  );
  const outboundPathId = await ensureRoutePath(
    line.id,
    "NORTE-IDA",
    "Ruta Norte · Ida",
    "IDA",
  ...
  const returnPathId = await ensureRoutePath(
    line.id,
    "NORTE-RETORNO",
    "Ruta Norte · Retorno",
    "RETORNO",
  ...
  const existingCalendar = await prisma.serviceCalendar.findFirst({
    where: { serviceLineId: line.id, name: calendarName },
  });
  const calendarData = {
  ...
  const calendar = existingCalendar
    ? await prisma.serviceCalendar.update({
        where: { id: existingCalendar.id },
        data: calendarData,
      })
    : await prisma.serviceCalendar.create({
        data: { serviceLineId: line.id, name: calendarName, ...calendarData },
      });
  ...
  const journey = await prisma.scheduleJourneyTemplate.findUniqueOrThrow({
    where: {
      scheduleTimeId_routePathId: {
        scheduleTimeId: outboundTimeId,
        routePathId: outboundPathId,
      },
    },
  });
  ...
  void student;
  console.log(
    `UPS GO demo ready for ${today}: Ruta Norte has one bus in ASSIGNED state.`,
  );
};
...
const cleanupDataset = async (): Promise<void> => {
  ...
  if (process.env["UPS_GO_DEMO_CONFIRM"] !== "DELETE") {
    throw new Error(
      "Set UPS_GO_DEMO_CONFIRM=DELETE to remove only the UPS GO demo dataset",
    );
  }
  ...
  const assignments = await prisma.serviceAssignment.findMany({
    where: { scheduledDepartureId: { in: departureIds } },
    select: { id: true },
  });
  ...
  await prisma.auditLog.deleteMany({
    where: {
      OR: [
        { entityId: { in: [...assignmentIds, ...runIds] } },
        { actorId: { in: userIds } },
      ],
    },
  });
  ...
  const calendar = await prisma.serviceCalendar.findFirst({
    where: { serviceLineId: line.id, name: calendarName },
  });
  if (calendar) {
    const patterns = await prisma.schedulePattern.findMany({
      where: { serviceCalendarId: calendar.id },
      select: { id: true },
    });
    const patternIds = patterns.map((pattern) => pattern.id);
    const times = await prisma.scheduleTime.findMany({
      where: { schedulePatternId: { in: patternIds } },
      select: { id: true },
    });
    const timeIds = times.map((time) => time.id);
    const journeys = await prisma.scheduleJourneyTemplate.findMany({
      where: { scheduleTimeId: { in: timeIds } },
      select: { id: true },
    });
    const journeyIds = journeys.map((journey) => journey.id);
    await prisma.scheduledStopTime.deleteMany({
      where: { journeyTemplateId: { in: journeyIds } },
    });
    ...
    await prisma.schedulePattern.updateMany({
  ...
  await prisma.stop.deleteMany({
    where: {
      name: { in: stopDefinitions.map((stop) => stop.name) },
      routePathStops: { none: {} },
    },
  });
...
void main().catch((error: unknown) => {
  console.error(
    error instanceof Error ? error.message : "UPS GO demo seed failed",
  );
  process.exitCode = 1;
});
